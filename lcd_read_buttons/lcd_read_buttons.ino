...
void setup(void) {
  ...
  analogWrite(backLight, 120); // turn backlight on. Replace 'HIGH' with 'LOW' to turn it off.
     ...
     // use 'buttonTEST' program on seperate file to display the button values or troubleshoot
{
 ...
 if (adc_key_in < 50)   {button = 5; return 5;}// SINGLE test             '>' button
 if (adc_key_in < 180)  {button = 3; return 3;}    // UP   test number scroll '^' button
 if (adc_key_in < 355)  {button = 4; return 4;} // DOWN test number scroll 'v' button
 if (adc_key_in < 530)  {button = 2; return 2;} // FAILed re-test          '>>' button
